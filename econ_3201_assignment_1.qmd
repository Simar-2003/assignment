...
(e) $\hat{g}(x)=\frac{\frac{1}{nh}\sum_{i=1}^ny_ik(\frac{x_i-x}{h})}{\frac{1}{nh}\sum_{i=1}^n     k(\frac{x_i-x}{h})}$
...
```{r}
u1 <- runif(n/2,0,1)   # min-0, max-1, length-500
u2 <- runif(n/2,0,1)   # min-0, max-1, length-500
```
